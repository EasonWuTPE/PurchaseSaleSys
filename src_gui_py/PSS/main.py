...
class Action():
	def __init__(self): 
		...
		try:
			self.FREAD = open( r"./Files/records.dat")  
			self.LINE_FREAD = [ LINES.rstrip() for LINES in self.FREAD ] 
		except IOError:
			self.LINE_FREAD= [] 
			pass 
	...
	def build_data(self):
		...
		while True:
			...
			if record_:
				...
				try: 
					...
					if len(self.LINE_FREAD)+1 != Record["ItemNo"]:
						print( "ERROR ItemNo.\n" ) 
						raise ValueError 
					self.LINE_FREAD.append( Record ) 
		...
		self.over_writing(self.LINE_FREAD) 
	...
	def sell_data(self):
		try:
			...
			self.LINE_FREAD[choice-1] = str( sell_update )
	...
	def delete_data(self):
		...
		while AccNum-1 >= len( self.LINE_FREAD ): 
		...
		self.LINE_FREAD.pop( AccNum-1 )
	
		# Modify the ItemNo after the deleted AccNum. 
		for i in self.LINE_FREAD[AccNum-1:]:   
			change_No = eval( i ) 
			change_No["ItemNo"] -= 1 
			self.LINE_FREAD[change_No["ItemNo"] - 1 ] = str(change_No) 
			#print( self.LINE_FREAD ) 
	...
	def insert_data():
		...
		self.LINE_FREAD.insert( AccNum-1, Record )
	
		# Modify the ItemNo after the inserted AccNum. 
		for i in self.LINE_FREAD[AccNum:]:   
			change_No = eval( i ) 
			change_No["ItemNo"] += 1 
			self.LINE_FREAD[change_No["ItemNo"]-1] = str(change_No )
			#print( self.LINE_FREAD ) 
	...
	def modified_data(): 
			...
			while True:
				...
				modify_reocrds = eval( self.LINE_FREAD[int(modify_[0])-1] ) 
				if modify_[1] in modify_reocrds.keys() :
					modify_reocrds[modify_[1]] = modify_[2] 
					self.LINE_FREAD[int(modify_[0])-1] = str( modify_reocrds ) 
...
if __name__=="__main__": 
	...
	# Main Program 
	action_ = Action() 
	action = { 1: action_.build_data,
			   2: action_.sell_data, 
		 	   3: action_.warning, 
			   4: action_.insert_data, 
			   5: action_.modified_data, 
			   6: action_.export_data,
			   7: action_.analysis_report, 
			   8: exit } 
